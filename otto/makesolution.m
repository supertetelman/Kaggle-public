...
if kmeans
	[ accuracy predict mapped ] = assess_kmeans(test(row,:), 0, centers, map);
	solution = [test(row,1) mapped];
	dlmwrite([num2str(length(centers)) '.solution.results.kmeans.csv'],[ test(row,1) (solution == 1:classifiers) ], '-append')
end
if (logistic && kmeans)
	for i=1:size(centers,1) %Run a sepperate regression against each cluster
		if predict == i
			this_theta = ((i-1)*classifiers+1):(i*classifiers);
			solution = [solution; [ test(i, 1)  log_predict(theta(:,this_theta),[1 test(i,2:end-1)], classifiers)]  ];
			break
		end
	end
	dlmwrite([num2str(length(centers)) '.solution.results.kmeans.and.logisitic.csv'],[ test(row,1) (solution == 1:classifiers) ])
end
